...
var extras = [[{x:0, y:0}], [{x:0, y:0}, {x:1, y:0}], [{x:0, y:0}, {x:1, y:0}, {x:2, y:0}], [{x:0, y:0}, {x:1, y:0}, {x:2, y:0}, {x:1, y: -3}], [{x:0, y:0}, {x:-4, y:-1}, {x:-2, y:-1}, {x:1, y:0}, {x:2, y:0}, {x:4, y:1}, {x:4, y:0}, {x:5, y:1}]];
...
function onMIDIAccess(inputs, outputs) {
  ...
  function updateLpNoteNames() {
    ...
    for (var y = -3; y < 5; y++) {
      for (var x = -5; x <= 3; x++) {
        ...
        lpMini.childNodes[i++].innerHTML = mode == 'play' ? (isMinor ? minors : majors)[pitchClass].name : '';
      }
    }
  }
  ...
  function noteLP(x, y, volume) {
    ...
    if (arpeggio) {
      ...
      if (volume) noteOn(note, {isMinor:isMinor}, ix, 127); else noteOff(ix);
    }
  ...
  function playArpeggio() {
    ...
    as.forEach(function(a) {
      var note = notes[a];
      if (note) {
        noteOn(note.note, note, note.ix, 127);
  ...
  function detectPort(port) {
    ...
    } else if (port.name.indexOf("Launchpad Mini") > -1) {
  ...
  function showLpMini() {
    ...
    function makeEl(x, y) {
      ...
      var ix = x + (y - 1) * 16;
      el.onmousedown = el.ontouchstart = y ? function() {
        lpMiniMsg({data: [0x90, ix, 127]});
      } : function() {
        lpMiniMsg({data: [0xb0, 104 + x, 127]});
      }
      el.onmouseup = el.ontouchend = y ? function() {
...
for (var tot = 18; tot >= 2; tot--) {
  for (var s = Math.max(1, tot - 9); s <= Math.min(9, tot - 1); s++) {
    var L = tot - s;
    octInfo[L*Lnum + s*snum] = L >= s ? `(L=${L}, s=${s})` : `(s=${L}, L=${s})`;
  }
}
